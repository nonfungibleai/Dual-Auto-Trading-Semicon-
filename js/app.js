...
function register(inputId, successId) {
  const inp = document.getElementById(inputId);
  const email = inp.value.trim();
  const re = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  if (!re.test(email)) { inp.style.borderColor='var(--red)'; inp.focus(); setTimeout(()=>inp.style.borderColor='',1500); return; }
  if (registered.has(email)) { document.getElementById(successId).className='success-box show'; return; }
  ...
  document.querySelectorAll('.h-stat-num em').forEach(el => {
    if (el.parentElement.nextElementSibling?.textContent.includes('사전')) el.textContent=(3241+list.length).toLocaleString();
  });
  inp.value='';
...
function animateCounter(el,target,duration){
  ...
  const update=(now)=>{
    ...
    el.innerHTML=Math.floor(e*target).toLocaleString();
